...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['pengeluaran'])){
		...
		$sql = 'DELETE FROM pengeluaran WHERE pengeluaran=?';
		$sql = 'DELETE FROM pengeluaran WHERE jumlah_pengeluaran=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=pengeluaran&&remove=hapus-data"</script>';
	}
	if(!empty($_GET['jual'])){
